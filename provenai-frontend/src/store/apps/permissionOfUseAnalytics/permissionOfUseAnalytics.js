...
export const updateGraphData = createAsyncThunk(
    ...
    async ( { providedDataTokens, consumedDataTokens } , thunkAPI) => {
        ...
        const providedDataTokensByOwnerDataPod = tokensByOwnerDataPod(providedDataTokens);
        const providedDataTokensByProcessorAgent = tokensByProcessorAgent(providedDataTokens);
        const providedDataTokensByDateTimeBucket = tokensByDateTimeBucket(providedDataTokens);

        const consumedDataTokensByOwnerDataPod = tokensByOwnerDataPod(consumedDataTokens);
        const consumedDataTokensByProcessorAgent = tokensByProcessorAgent(consumedDataTokens);
        const consumedDataTokensByDateTimeBucket = tokensByDateTimeBucket(consumedDataTokens);
...
export const fetchAnalytics = createAsyncThunk(
    ...
    async ({ organizationId, storedToken }, thunkAPI) => {
        try {
            ...
            const providedDataPromise = permissionOfUseAnalyticsService.getAnalyticsByCriteria({ ownerOrganizationId: organizationId, ...filters.apiFilters }, storedToken);
            const consumedDataPromise = permissionOfUseAnalyticsService.getAnalyticsByCriteria({ processorOrganizationId: organizationId, ...filters.apiFilters }, storedToken);

            // Use Promise.all to wait for both promises to resolve
            const [providedData, consumedData] = await Promise.all([providedDataPromise, consumedDataPromise]);
...
export const permissionOfUseAnalyticsActions = permissionOfUseAnalyticsSlice.actions;
