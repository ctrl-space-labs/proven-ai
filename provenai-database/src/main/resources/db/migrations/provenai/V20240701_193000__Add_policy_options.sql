...
SELECT pt.id, 'ALLOW_DATA_POD_NAME', 'Policy option for allowing specific agents access to a data pod'
...
WHERE pt.name = 'ALLOW_LIST'
  AND NOT EXISTS (
    ...
    WHERE po.policy_type_id = pt.id
      AND po.name = 'ALLOW_DATA_POD_NAME'
);

-- Insert DENY_AGENT_NAME policy option
INSERT INTO proven_ai.policy_options (policy_type_id, name, description)
SELECT pt.id, 'DENY_DATA_POD_NAME', 'Policy option for denying specific agents access to a data pod'
FROM proven_ai.policy_types pt
WHERE pt.name = 'DENY_LIST'
  AND NOT EXISTS (
    ...
    FROM proven_ai.policy_options po
    WHERE po.policy_type_id = pt.id
      AND po.name = 'DENY_DATA_POD_NAME'
);